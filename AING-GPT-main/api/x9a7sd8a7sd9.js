...
export default async function handler(req, res) {
  ...
  for (let i = 0; i < apiKeys.length; i++) {
    ...
    const externalApiUrl = `https://generativelanguage.googleapis.com/v1beta/models/${GEMA_MODEL}:generateContent?key=${currentKey}`;

    try {
      // console.log(`[Attempt] Using Key Index: ${i} for Model: ${GEMA_MODEL}`); // Uncomment untuk debug
