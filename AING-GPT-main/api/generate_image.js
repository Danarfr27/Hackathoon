...
export default async function handler(req, res) {
  ...
  const keysString = process.env.IMAGE_API_KEYS || process.env.GENERATIVE_API_KEY || process.env.GEMA_API_KEYS || '';
  ...
  const provider = (process.env.IMAGE_PROVIDER || 'gema').toLowerCase();
  const GEMA_IMAGE_MODEL = process.env.GEMA_IMAGE_MODEL || process.env.GEMA_MODEL || 'gema-2.5-flash-image';
  ...
  for (let i = 0; i < apiKeys.length; i++) {
    ...
    try {
      if (provider === 'openai') {
        ...
        // Gema / Google Generative Images via model endpoint
        // Prefer model-specific endpoint when a model is provided: /v1beta/models/{model}:generateImage
        const model = (req.body && req.body.model) ? String(req.body.model).trim() : GEMA_IMAGE_MODEL;
        ...
        // Gema expects a prompt object and image config; we provide size as-is
        const body = {
          prompt: { text: prompt },
          image: { size }
        };
        ...
        if (r.ok) {
          // Typical Gema image response may include `images`, `data`, `imageUri` or `artifacts`
          finalData = await r.json();
          success = true;
          break;
        }
        ...
        if (r.status === 404) {
          ...
          detail.hint = detail.hint || `Verify model name ('${GEMA_IMAGE_MODEL}' or provided '${model}'), enable Generative AI API, and ensure the key has Image generation permission.`;
          lastError = { status: r.status, statusText: r.statusText, provider: 'gema', attempt: i, details: detail };
          break;
        }

        if (r.status === 429) {
          lastError = { status: 429, message: 'Rate limit', provider: 'gema', attempt: i };
          continue;
        }
        ...
        let errB = {};
        try { errB = txt ? JSON.parse(txt) : {}; } catch (e) { errB = { text: txt }; }
        lastError = { status: r.status, statusText: r.statusText, provider: 'gema', attempt: i, details: errB };
        break;
      }
    } catch (err) {
